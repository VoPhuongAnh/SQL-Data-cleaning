...
### Top 10 first records from dataset
...
## How to copy a table step by step

### Step 1: Generate a clone table from existing table
```SQL
...
```
### Step 2: Copy all values from original table to new clone table

```SQL
INSER INTO club_member_info_cleaned
SELECT * FROM club_member_info
```

## Alright, now we continue cleaning the table

### Start with formating the full_name clolumn

-- loc lay cac record age tu 0 den 100 :
SELECT UPPER(TRIM(full_name)), age, martial_status, email,phone, full_address, job_title, membership_date
FROM club_member_info_cleaned
--WHERE (age BETWEEN 0 AND 100) OR (age IS NULL)
ORDER BY age DESC;

### Format full_name column cho cac record co age < 100:

```SQL
-- loc lay cac record co age < 100: 

SELECT UPPER(TRIM(full_name)), substring(age, 1, 2) AS cleaned_age, martial_status , email , phone , full_address , job_title , membership_date 
FROM club_member_info_cleaned
WHERE age < 100
ORDER BY age DESC;
```
...
|UPPER(TRIM(full_name))|cleaned_age|martial_status|email|phone|full_address|job_title|membership_date|
|----------------------|-----------|--------------|-----|-----|------------|---------|---------------|
...
### Format full_name column cho cac record co age > 100 (outliers) va empty:

```SQL
-- clean cot full_name, age cua cac record co age > 100 va empty:
...
### Join hai table voi nhau:

```SQL
SELECT UPPER(TRIM(full_name)), substring(age, 1, 2), martial_status , email , phone , full_address , job_title , membership_date
	FROM club_member_info_cleaned WHERE age > 100
UNION
SELECT UPPER(TRIM(full_name)), substring(age, 1, 2), martial_status , email , phone , full_address , job_title , membership_date 
	FROM club_member_info_cleaned WHERE age < 100;
```
